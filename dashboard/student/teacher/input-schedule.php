...
$teacher = new Teacher($conn, $teacher_id);
// Retrieve schedules
...
<script>
    ...
    // Disable based on guidances
    guidances.forEach(guidance => {
        const guidanceDate = new Date(guidance.date);
        if (isDatesEquals(guidanceDate, date)) {
...
if (isset($_POST["request-button"])) {
    ...
    if ($pertemuan->requestPertemuan($teacher_id, $formatted_date, $selected_time)) {
